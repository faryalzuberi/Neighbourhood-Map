...
var locationModel = function(location) { 
	...
	this.getVenuePhotos = function(id) {
          ...
          $.get({
           url: baseUrl,
           success: function(response) {
            	self.photo(response.response.photos.items[0].prefix + 'original' + response.response.photos.items[0].suffix);
           }
         });
      }
   ...
   $(window).on('load', function() { 
   	var ViewModel = function() {
	    ...
	    self.showInfoWindow = function(location) {
	  	...
	  	//called when the user clicks the search button in the view
	  	self.filterLocations = function(input) {
	  	...
	  	//and adds it to our observable array
	  	self.addLocations = function() {
	  		locations.forEach(function(location){
				self.locationsObservableArray.push(new locationModel(location));
			});
	  	}

	  	//calls when the user clicks reset in the ui
	  	//this is to rest the markers and list if the user
	  	//makes a search
